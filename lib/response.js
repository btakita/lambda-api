...
class RESPONSE {
  ...
  send(body) {
    console.debug('send|debug|1')
    console.debug(body)
    console.trace(new Error().stack.replace('\n', '|'))
    console.debug('send|debug|1.1')
    console.debug(JSON.stringify({
      'this._request.originalUrl': this._request.originalUrl,
      'this._request._parsedOriginalUrl': this._request._parsedOriginalUrl,
    }))
    // Generate Etag
...
console.debug('send|debug|6')
    // Trigger the callback function
    this.app._callback(null, this._response, this)
console.debug('send|debug|7')
  } // end send
